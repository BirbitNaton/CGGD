...
ComPtr<IDXGIFactory4> cg::renderer::dx12_renderer::get_dxgi_factory()
{
	...
	if (SUCEEDED(D3D12GetDebugInterface(IID_PPV_ARGS(&debug_controller))))
	{
...
void cg::renderer::dx12_renderer::create_render_target_views()
{
	rtv_heap.create_heap(
			device, D3D12_DESCRIPTOR_HEAP_TYPE_RTV, frame_number
	);
...
void cg::renderer::dx12_renderer::create_command_list()
{
	THROW_IF_FAILED(device->CreateCommandList(
			0,
			D3D12_COMMAND_LIST_TYPE_DIRECT, command_allocators[0].Get(), pipeline_state.Get(),
...
void cg::renderer::dx12_renderer::create_root_signature(const D3D12_STATIC_SAMPLER_DESC* sampler_descriptors, UINT num_sampler_descriptors)
{
	...
	rs_desc.Init_1_1 (
			_countof (root_parameters), root_parameters, num_sampler_descriptors, sampler_descriptors,
...
ComPtr<ID3DBlob> cg::renderer::dx12_renderer::compile_shader(const std::filesystem::path& shader_path, const std::string& entrypoint, const std::string& target)
{
	...
	HRESULT result = D3DCompileFromFile (
			...
			nullptr, nullptr, entrypoint.c_str(), target.c_str(), compile_flags,
			0, &shader, &error
	);
	if (FAILED (result))
	{
...
void cg::renderer::dx12_renderer::create_pso(const std::string& shader_name)
{
	...
	D3D12_INPUT_ELEMENT_DESC input_descsriptors[] = {
			{"POSITION", 0,
	...
	D3D12_GRAPHICS_PIPELINE_STATE_DESC pso_desc{};
		pso_desc.InputLayout = {input_descsriptors, _countof(input_descsriptors)};
...
void cg::renderer::dx12_renderer::load_assets()
{
		...
		for (size_t i = 0; i < model->get_index_buffers().size(); i++) {
			...
			create_resource_on_upload_heap(vertex_buffers[i],
										   vertex_buffer_size, vertex_buffer_name);

			copy_data(vertex_buffer_data->get_data(), vertex_buffer_size,
					  vertex_buffers[i]);

			vertex_buffer_views[i] = create_vertex_buffer_view(
					vertex_buffers[i], vertex_buffer_size);
			...
			create_resource_on_upload_heap(index_buffers[i],
										   index_buffer_size, index_buffer_name);
			copy_data(index_buffer_data->get_data(), index_buffer_size,
					  index_buffers[i]);

			index_buffer_views[i] = create_index_buffer_view(
					index_buffers[i], index_buffer_size);
		}
...
void cg::renderer::dx12_renderer::populate_command_list()
{
		...
		command_list->ResourceBarrier (
				_countof (begin_barriers), begin_barriers
		);
		...
		command_list->ClearRenderTargetView(
				rtv_heap.get_cpu_descriptor_handle(frame_index), clear_color,
				0,
				nullptr
		);
